...
if (mode=="rail")sql<-"SELECT source, target, length, 1.0 as stops FROM graph.transit_multimodal_randstad WHERE rail=TRUE AND transfer=0"
if (mode=="tram")sql<-"SELECT source, target, length, 1.0 as stops FROM graph.transit_multimodal_randstad WHERE tram=TRUE AND transfer=0"
if (mode=="metro")sql<-"SELECT source, target, length, 1.0 as stops FROM graph.transit_multimodal_randstad WHERE metro=TRUE AND transfer=0"
if (mode=="bus")sql<-"SELECT source, target, length, 1.0 as stops FROM graph.transit_multimodal_randstad WHERE bus=TRUE AND transfer=0"
if (mode=="transit")sql<-"SELECT source, target, length+0.00001 length, temporal, 1.0 as stops, transfer+1.0 as transfer FROM graph.transit_multimodal_randstad"
if (mode=="tracks")sql<-"SELECT source, target, length+0.00001 length, temporal, 1.0 as stops, transfer+1.0 as transfer FROM graph.transit_multimodal_randstad WHERE bus IS NULL"
