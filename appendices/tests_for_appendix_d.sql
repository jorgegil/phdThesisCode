...
-- using building IDs
CREATE TABLE distances.sources AS SELECT multimodal_sid sid, the_geom::geometry(MultiPolygon, 28992) FROM urbanform.buildings_randstad WHERE multimodal_sid IN (1993893,2042963,2002182,2002995);
SELECT populate_geometry_columns('distances.sources'::regclass,false);
...
CREATE TABLE distances.routes (sid serial NOT NULL PRIMARY KEY, the_geom geometry(Linestring, 28992), node_id integer, link_id integer, dist double precision, od text, distance_type text);
SELECT populate_geometry_columns('distances.routes'::regclass,false);
-- DELETE FROM distances.routes;
-- test
SELECT seq, id1 AS node, id2 AS edge, cost FROM pgr_dijkstra('SELECT sid AS id,source::int4, target::int4, axial::float8 AS cost FROM temp_pedestrian_graph', (SELECT entrance FROM distances.sources WHERE code='A')::int, (SELECT entrance FROM distances.sources WHERE code='B')::int, false, false);
-- add one
WITH this_route AS (SELECT seq, id1 AS node, id2 AS edge, cost FROM pgr_dijkstra('SELECT sid AS id,source::int4, target::int4, metric::float8 AS cost FROM temp_pedestrian_graph', (SELECT entrance FROM distances.sources WHERE code='A')::integer, (SELECT entrance FROM distances.sources WHERE code='B')::integer, false, false))
...
CREATE TABLE distances.service_areas (sid serial NOT NULL PRIMARY KEY, the_roads geometry(Linestring, 28992), the_buildings geometry(MultiPolygon, 28992), node_id integer, dist double precision, origin text, distance_type text, max_distance double precision);
SELECT populate_geometry_columns('distances.service_areas'::regclass,false);
CREATE INDEX service_areas_node_idx ON distances.service_areas (node_id);
-- DELETE FROM distances.service_areas;
-- test
SELECT seq, id1 AS node, cost FROM pgr_drivingDistance('SELECT sid AS id,source::int4, target::int4, metric::float8 AS cost FROM temp_pedestrian_graph', (SELECT entrance FROM distances.sources WHERE code='D')::integer, 400.0::double precision, false, false);
